...
// ---- the fractal NARROW PHASE (v9.9) --------------------------------------
// hullDist above is the rounded BOX. The hull actually drawn is that box
// INTERSECTED with a mandelbox (shipDE in the shader), so parts of the box are
// genuinely empty: measured at the shipped tuning, 3.9% of the mothership and
// 8.7% of a harvester (node test/hull_census.js). Colliding against the box
// alone bounced the craft off holes it could see straight through.
//
// hullDist stays the BROAD phase and this runs only once the craft is already
// inside the box, so the common case costs nothing -- a handful of these per
// second, against 8 iterations of plain arithmetic each.
//
// ⚠️ THIRD copy of shaping maths, after the shader and terrain.js. It has to
// track uBoxParam exactly or the craft collides with a different ship than the
// one drawn; test_aliens.js pins every constant against the GLSL text.
const MB_ITERS = 8;        // matches the shader's mandelboxDE loop
const MB_FIXR2 = 1.0;      // the outer sphere-fold radius, squared
const MB_STRETCH = 1.15;   // shipDE's conservative factor for the non-uniform squash

// Mirrors mandelboxDE(p, minR2). Returns a distance ESTIMATE that is >= 0
// everywhere and tends to 0 inside the solid -- it never goes negative.
function mandelboxDEJ(px, py, pz) {
  const S = TUNEA.boxScale.v, F = TUNEA.boxFold.v;
  const minR2 = TUNEA.boxMinR.v * TUNEA.boxMinR.v;
  let qx = px, qy = py, qz = pz, dr = 1.0;
  for (let i = 0; i < MB_ITERS; i++) {
    qx = Math.min(Math.max(qx, -F), F) * 2 - qx;        // box fold
    qy = Math.min(Math.max(qy, -F), F) * 2 - qy;
    qz = Math.min(Math.max(qz, -F), F) * 2 - qz;
    const r2 = qx * qx + qy * qy + qz * qz;
    if (r2 < minR2)      { const k = MB_FIXR2 / minR2; qx *= k; qy *= k; qz *= k; dr *= k; }
    else if (r2 < MB_FIXR2) { const k = MB_FIXR2 / r2; qx *= k; qy *= k; qz *= k; dr *= k; }
    qx = qx * S + px; qy = qy * S + py; qz = qz * S + pz;
    dr = dr * Math.abs(S) + 1;
  }
  return Math.hypot(qx, qy, qz) / Math.abs(dr);
}

// Mirrors shipDE(l, h, minR2): the box and the fractal, intersected.
function shipDEJ(lx, ly, lz, half) {
  const hmin = Math.min(half[0], Math.min(half[1], half[2]));
  const mb = mandelboxDEJ(lx / half[0] * MB_STRETCH,
                          ly / half[1] * MB_STRETCH,
                          lz / half[2] * MB_STRETCH) / MB_STRETCH * hmin;
  return Math.max(hullDist(lx, ly, lz, half), mb);
}

// Metres of contact band. The DE is NOT signed (see above), so "in material"
// cannot be `< 0`; it is `< HULL_SKIN`. Sized at roughly the craft's half-width
// so the fuselage crashes when it touches solid, not when its centre does.
const HULL_SKIN = 2.0;

// THE collision predicate for a box hull.
//
// The hullDist line is a COST gate, not a correctness one: shipDEJ already
// takes max() with the same box distance, so removing it changes no answer,
// only how often the 8-iteration fractal runs. Said plainly because a mutation
// of it escapes the suite, and the next person should know that is correct
// rather than a hole in the tests.
export function hullSolidAt(lx, ly, lz, half, skin) {
  if (hullDist(lx, ly, lz, half) > (skin === undefined ? HULL_SKIN : skin)) return false;
...
export function updateAliens(dt, now) {
  ...
  if (DBG.invasion > 0.5) {
    if (hullAlive(m) && hullSolidAt(cp[0] - m.x, cp[1] - m.y, cp[2] - m.z, motherHalf())) doCrash('ALIEN HULL');
    ...
    for (const s of alien.ships) {
      ...
      const lx = ox * ca - oz * sa, lz = ox * sa + oz * ca;
      if (hullSolidAt(lx, cp[1] - s.y, lz, shipHalf())) { doCrash('ALIEN HULL'); break; }
    }
  }
